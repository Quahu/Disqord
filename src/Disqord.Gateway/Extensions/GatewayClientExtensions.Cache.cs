...
public static partial class GatewayClientExtensions
{
    ...
    public static CachedMember? GetCurrentMember(this IGatewayClient client, Snowflake guildId)
    {
        var currentUser = client.CurrentUser;
        if (currentUser != null && client.CacheProvider.TryGetMembers(guildId, out var cache, true))
            return cache.GetValueOrDefault(currentUser.Id);
